    ...
    if (file !== undefined) {
      if (!_.isNil(this.allowMaxSize) && this.allowMaxSize !== '') {
        if (file.size > parseFloat(this.allowMaxSize) * 1024 * 1024) {
          Notification.show(
            'warning',
            `Please select file less than ${this.allowMaxSize}MB`,
            5000
          );
          return false;
        }
      ...
      if (
        !_.isUndefined(this.accept) &&
        _.isUndefined(
          _.find(
            this.valid_extensions,
            item =>
              item.toLowerCase() === this.getFileExtension(file).toLowerCase()
          )
        )
      ) {
        Notification.show(
          'warning',
          `Please select a file with valid type`,
          5000
        );
        return false;
      }
